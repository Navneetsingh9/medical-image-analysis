...
def save_report(image_name, report, model_used, findings_preview=None):
    ...
    try:
        ...
        report_data = {
            ...
            'report': report,
            'findings_preview': findings_preview or report[:200] + '...'
        }
        
        # Create filename with timestamp
        safe_image_name = image_name.replace(' ', '_').replace('.', '_')
        filename = f"{REPORTS_DIR}/report_{timestamp.strftime('%Y%m%d_%H%M%S')}_{safe_image_name}.json"
